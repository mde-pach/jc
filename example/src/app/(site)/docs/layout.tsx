    ...
    <div className="max-w-6xl mx-auto px-6 flex gap-12 py-12 min-h-[calc(100vh-56px-73px)]">
      <aside className="hidden md:block w-48 shrink-0">
        <nav className="sticky top-[80px] flex flex-col gap-0.5">
          <span className="text-[11px] font-semibold text-fg-subtle uppercase tracking-widest mb-3 px-3 font-mono">
            Docs
          </span>
          {sections.map((s) => (
            <a
              key={s.href}
              href={s.href}
              className="px-3 py-2 text-[13px] font-medium text-fg-muted hover:text-fg hover:bg-surface-overlay rounded-lg transition-colors no-underline"
            >
              {s.label}
            </a>
          ))}
